...
void getBobParameters(mpz_t r, mpz_t y, mpz_t x, const mpz_t p, const mpz_t q, const mpz_t n) {
    // set r
    ...
    } while ((mpz_divisible_p(pmo, r) == 0) || (mpz_cmp_ui(gcdB, 1) != 0) || (mpz_cmp_ui(gcdC, 1) != 0));
...
void benalohDecrypt(mpz_t decrypted, const mpz_t ciphertext, const mpz_t p,
                    const mpz_t q, const mpz_t r, const mpz_t x, const mpz_t n) {
...
//   generateDistinctPrimes(p, q, 9);
  mpz_set_ui(r, 99);  // r = 3
//   mpz_set_ui(x, 24640);
//   mpz_set_ui(y, 13213);    // y = 5
//   mpz_set_ui(u, 66183);
